...
class FactPlausibilityTrainer : public LearnerTask {
 public:
  ...
  void Run(task::Task *task) override {
    ...
    task->Fetch("sections", &num_sections_);
    task->Fetch("instances_per_section", &instances_per_section_);
    ...
    for (int i = 0; i < num_sections_; ++i) {
      training_sections_.push_back(new Handles(&store_));
    }
    while (input.Read(&message)) {
      ...
      if (groups.length() >= min_facts_) {
        ...
        if (eval_ratio_ == 0 || rnd.UniformProb() > eval_ratio_) {
          int section = instance.GetInt(p_section_);
          if (section >= num_sections_) section = 0;
          training_sections_[section]->push_back(instance.handle());
    ...
    for (int i = 0; i < num_sections_; ++i) {
      LOG(INFO) << "training section " << i << ": "
                << training_sections_[i]->size() << " instances";
    }
    store_.Freeze();
    ...
    for (auto *section : training_sections_) delete section;
    delete optimizer_;
  }
    ...
    for (;;) {
      ...
      for (int b = 0; b < batches_per_update_; ++b) {
        ...
        for (int i = 0; i < batch_size_; ++i) {
          if (instances_per_section_ != 0) {
            if (i % instances_per_section_ == 0) {
              // Sample section.
              section = rnd.UniformInt(num_sections_);
            }
          }

          int sample = rnd.UniformInt(training_sections_[section]->size());
          Frame instance(&store_, (*training_sections_[section])[sample]);
        ...
        // generated using the premise from one item and the hypothesis from
        // another item.
        for (int i = 0; i < batch_size_; ++i) {
 ...
 private:
  ...
  void Split(const Frame &instance,
    ...
    for (int g = 0; g < num_groups; ++g) {
      ...
      start = end;
      end = groups.get(g).AsInt();

      if (g == heldout) {
        // Add fact group to hypothesis.
  ...
  int num_sections_ = 4;               // number of training data sections
  int instances_per_section_ = 2;      // instances to sample from each section
  int batches_per_update_ = 256;       // number of batches per epoch
  float eval_ratio_ = 0.001;           // train/eval split ratio
  float learning_rate_ = 1.0;          // learning rate
  float min_learning_rate_ = 0.001;    // minimum learning rate
  ...
  // Training instances by section.
  std::vector<Handles *> training_sections_;
