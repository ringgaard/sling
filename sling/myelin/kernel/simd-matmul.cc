    ...
    for (auto &phase : strategy.phases()) {
      ...
      if (phase.repeat > 1) {
        ...
        for (int i = 0; i < phase.unrolls; ++i) {
          ...
          bool keep = i != phase.unrolls - 1;
          gen->MultiplyAdd(sum[i], elem, Operand(b_ptr, disp), keep);
        }
        ...
        __ bind(&l3);
        gen->Broadcast(elem, Operand(a, a_ofs));
        gen->MaskedMultiplyAdd(sum[0], elem, Operand(b_ptr));
