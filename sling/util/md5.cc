...
// The algorithm is due to Ron Rivest. The code was written by Colin Plumb in
// 1993, and place into the the public domain with no copyright is claimed.
...
void MD5Update(struct MD5Context *ctx, const unsigned char *buf, size_t len) {
	// Update bitcount.
	uint32 t = ctx->bits[0];
	ctx->bits[0] = (t + (len << 3)) & 0xffffffff;
	if (ctx->bits[0] < t) ctx->bits[1]++;  // carry from low to high
	ctx->bits[1] += len >> 29;
	t = (t >> 3) & 0x3f;
...
void MD5Final(unsigned char digest[16], MD5Context *ctx) {
	...
	*p++ = 0x80;
	...
	// Clear context in case it's sensitive.
...
void MD5Digest(unsigned char digest[16], const void *buf, size_t len) {
  MD5Context ctx;
  MD5Init(&ctx);
  MD5Update(&ctx, reinterpret_cast<const unsigned char *>(buf), len);
  MD5Final(digest, &ctx);
}
